...
生产入口使用 `https://digitalleft.org`。

Worker 通过 Wrangler `routes` 绑定两个自定义域名：

- `digitalleft.org`
- `www.digitalleft.org`

Better Auth 的生产 `baseURL` 同步改为 `https://digitalleft.org`。GitHub OAuth App 的回调地址应配置为：

```txt
https://digitalleft.org/api/auth/callback/github
```

## Consequences

- 学员访问和注册优先使用项目自有域名，不再依赖 `workers.dev`。
