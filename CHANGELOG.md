...
- 课程反馈地基：课程页对已解锁学员显示反馈区，反馈必须选择状态并填写文字；反馈同步到课程讨论帖，后台可看统计与撤回反馈。
- 每日摘要邮件：默认开启，按 Asia/Shanghai 上一自然日汇总关注帖新回复和本人回复获赞，支持用户中心开关、邮件退订和发送幂等。
- MCP 新增课程反馈、关注帖子、回复点赞工具，复用网页同一服务层和权限检查。

### 变更

- 课程导入脚本在发布课程时同步维护课程讨论帖；新增 `course:discussions:backfill` 用于补齐历史已发布课程的讨论帖。

### 计划

- `0.4.0` 剩余主线：用真实学员账号做课程反馈、论坛互动、每日摘要和 MCP 全链路验收。
